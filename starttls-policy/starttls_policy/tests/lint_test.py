...
class TestLint(unittest.TestCase):
    ...
    def test_lint_contents(self):
        lol = ("{ \"a\": \"b\"\n"
               "  \"c\": 3,\n"
               "  \"d\": \"x\" }\n")
        self.assertRaises(ValueError, lint.lint_contents, lol)

    def test_lint_file(self):
        lol = ("{ \"timestamp\": 0, \"expires\": 0 }")
        with mock.patch('starttls_policy.lint.open', mock.mock_open(read_data=lol)):
            lint.validate("fake_file")
    ...
    def test_pretty_print_out_of_bounds(self):
        ...
        lol = ("{ \"a\": \"b\"\n"
               "  \"c\": 3,\n"
               "  \"d\": \"x\" }\n")
        self.assertEqual("", lint._pretty_print_relevant_lines(lol, 4, 4))

    def test_describe_json_decode_malformed(self):
        # pylint: disable=protected-access
        self.assertRaises(ValueError, lint._describe_json_decode_error,
